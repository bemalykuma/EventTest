  ...
  <div class="bg-white rounded-[20px] border-2 border-black w-[250px] h-132 p-3 flex flex-col">
    <div>
      <div class="w-full h-78 bg-gray-200 rounded-[10px] overflow-hidden">
        ...
        <p class="text-sm text-red-500 mt-2 font-medium">{{ formattedDate }}</p>
    
        <h1 class="font-bold mt-1 text-[16px] leading-snug line-clamp-2">
          {{ props.event.name }}
        </h1>
    </div>
    <div class="flex-1 flex flex-col justify-end">
        ...
        <div
          class="flex items-center gap-1 mt-1 text-sm flex-row"
        ...
        <div class="flex gap-1 mt-3">
            <NuxtLink :to="`/events/${props.event.id}`" class="flex-1 ">
              <Button class=" rounded-[10px] w-full cursor-pointer ">ดูรายละเอียด</Button>
            </NuxtLink>
        </div>
    </div>
  </div>
